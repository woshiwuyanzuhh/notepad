...
### 测试状态
- 前端 Vitest：**待本次修改后重跑**（`pnpm test`）
- Rust：**33 通过**（`cargo test`，src-tauri 目录下；本次未改 Rust）
- `pnpm build` / `pnpm tauri build`：**待本次修改后重跑**；clippy 零警告
...
1. **用户最近反馈"界面乱"**——已定位并修复（CSS 缺失 + 类名不匹配 + 缓存 + 格式工具条 flex + 编辑器空状态居中 + JSON 工具条/压缩/光标/分栏拖拽），**待用户确认新安装版效果**。若仍乱，让用户提供截图文件路径，用能读图的模型查看。
